...
if(X=="Microsoft Internet Explorer")
	...
	G=true;
A=$.$dpPath||J();if($.$wdate)K(A+"skin/WdatePicker.css");
V=Y;
if($.$crossFrame){
	try{
...
Y[N].docMD=true}if(!V[N].docMD){E(V[N],"onmousedown",D,true);
V[N].docMD=true}E(Y,"onunload",function(){if($dp.dd)P($dp.dd,"none")});
...
function U(K,C){
	...
	if(C){
		if(!J()){
			_=_||setInterval(function(){if(V[N].readyState=="complete")clearInterval(_);U(null,true)},50);
			return
		}
		if($dp.status==0){
			$dp.status=1;
		...
		var F=D();
		if(Y.event===F||F){
			L.srcEl=F.srcElement||F.target;F.cancelBubble=true
		}
	...
	function J(){
		if(S&&V!=Y&&V[N].readyState!="complete")return false;return true
	}
	function D(){
		if(G){func=D.caller;while(func!=null){var $=func.arguments[0];if($&&($+"").indexOf("Event")>=0)return $;func=func.caller}return null}return event
	}
}
...
function popDate(){
	alert(0);
	WdatePicker({dateFmt: 'yyyMMdd'});
}

function popDate2(obj){
	WdatePicker({dateFmt: 'yyyMMdd', el: obj.id});
}